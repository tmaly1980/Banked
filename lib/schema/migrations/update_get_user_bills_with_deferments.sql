-- Update user_bills_view to include active deferment information
-- This allows bills to show active_deferred_months array with decide_by_dates and loss_dates
-- View returns bills for all users, filter by user_id in WHERE clause when querying
...
WITH bill_payment_summary AS (
  ...
  GROUP BY bp.bill_id, bp.user_id
),
current_period_payments AS (
  ...
  FROM bill_payments bp
  WHERE COALESCE(bp.applied_month_year, TO_CHAR(bp.payment_date, 'YYYY-MM')) = TO_CHAR(CURRENT_DATE, 'YYYY-MM')
  GROUP BY bp.bill_id, bp.user_id
),
...
bill_deferments_agg AS (
  ...
  WHERE bd.is_active = false -- Only include inactive/historical deferments
  GROUP BY bd.bill_id, bd.user_id
),
...
bill_instances AS (
  ...
  LEFT JOIN bill_deferments_agg bd ON b.id = bd.bill_id AND b.user_id = bd.user_id
  LEFT JOIN active_deferments ad ON b.id = ad.bill_id AND b.user_id = ad.user_id
  WHERE b.due_date IS NOT NULL

  UNION ALL

  -- Recurring bills - current month instance
  SELECT 
    b.id,
    b.user_id,
    b.name,
  ...
  WHERE b.due_day IS NOT NULL
    ...
    -- Exclude instances for months that are actively deferred
    AND (ad.active_deferred_months IS NULL OR NOT (TO_CHAR(CURRENT_DATE, 'YYYY-MM') = ANY(ad.active_deferred_months)))
  ...
  WHERE b.due_day IS NOT NULL
    ...
    -- Exclude instances for months that are actively deferred
    AND (ad.active_deferred_months IS NULL OR NOT (TO_CHAR(CURRENT_DATE + INTERVAL '1 month', 'YYYY-MM') = ANY(ad.active_deferred_months)))
...
SELECT
  ...
  CASE 
    ...
    ELSE (bi.instance_due_date - CURRENT_DATE::DATE)
  END as days_until_due,
  CASE 
    WHEN bi.instance_due_date IS NULL THEN false
    WHEN bi.instance_due_date < CURRENT_DATE::DATE THEN true
    ELSE false
  END as is_overdue,
  CASE
    WHEN bi.instance_due_date >= CURRENT_DATE::DATE THEN true
